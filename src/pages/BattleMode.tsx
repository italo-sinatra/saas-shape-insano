...
import { Swords, Timer, ArrowLeft, Check, Flame } from "lucide-react";
...
const BattleMode = () => {
  ...
  const allDone = exerciseList.every((e) => e.done);
  ...
  return (
    <div className="min-h-screen bg-background p-4 max-w-lg mx-auto">
      ...
      >
        <div className={`inline-block rounded-2xl px-8 py-4 border-2 ${
          running ? "border-primary animate-pulse-glow" : "border-border"
        }`}>
        ...
        {exerciseList.map((ex, i) => (
          ...
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
              ex.done ? "gold-gradient" : "bg-secondary"
            }`}>
              {ex.done ? <Check size={16} className="text-accent-foreground" /> : <span className="text-xs text-muted-foreground">{i + 1}</span>}
            </div>
            <div className="flex-1">
              <p className={`text-sm font-semibold ${ex.done ? "text-accent line-through" : "text-foreground"}`}>
                {ex.name}
              </p>
      ...
      {complete && (
        ...
        >
          ...
          <p className="text-accent text-sm font-cinzel mb-6">+350 XP • +50 Dracmas</p>
          <motion.button
